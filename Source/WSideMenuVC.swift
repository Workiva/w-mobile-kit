...
public class WSideMenuContentVC: UIViewController, WSideMenuProtocol {
  ...
  public func addWSideMenuButtons() {
    ...
    let sideMenuButtonItem = UIBarButtonItem(title: "Toggle", style: .Plain, target: self, action: #selector(WSideMenuContentVC.toggleSideMenu))
    
    if (navigationController?.viewControllers.count > 1) {
      let backMenuButtonItem = UIBarButtonItem(title: "Back", style: .Plain, target: self, action: #selector(WSideMenuContentVC.backButtonItemWasTapped(_:)))
